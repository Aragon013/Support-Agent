...
import type { InMemorySessionEventBus, SessionEvent } from "../domain/session-event-bus.js";
...
export class SessionEventsWsHub {
  private readonly clients = new Set<WsClient>();
  private readonly recent: SessionEvent[] = [];
  private readonly seen = new Set<string>();

  constructor(private readonly maxRecent = 500) {}

  attach(eventBus: InMemorySessionEventBus): () => void {
  ...
  addClient(socket: WebSocket, filter: WsFilter): void {
    ...
    for (const event of this.recent) {
      if (event.seq <= filter.sinceSeq) continue;
      if (!matchesFilter(event, filter)) continue;
      socket.send(JSON.stringify({ v: 1, type: "session.event", event }));
    }
  ...
  publish(event: SessionEvent): void {
    ...
    const msg = JSON.stringify({ v: 1, type: "session.event", event });
...
function matchesFilter(event: SessionEvent, filter: WsFilter): boolean {
  if (event.tenantId !== filter.tenantId) return false;
  if (filter.endpointId && event.endpointId !== filter.endpointId) return false;
  if (filter.operatorId && event.operatorId !== filter.operatorId) return false;
  return true;
}
...
export function registerSessionEventsWsRoute(
  app: FastifyInstance,
  hub: SessionEventsWsHub,
): void {
  app.route({
    method: "GET",
    url: "/api/v1/sessions/events/ws",
